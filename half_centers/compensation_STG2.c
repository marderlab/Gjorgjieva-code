//25 Sept 2015: this actually won't work because too many large arrays so C generates Segmentation Fault. See compensation_STG3.c
...
int main(int argc, char *argv[])
{
  ...
  double V[300001];
  double taumNa[300001], tauhNa[300001], taumCaT[300001], tauhCaT[300001], taumCaS[300001], tauhCaS[300001], taumA[300001], tauhA[300001], taumKCa[300001], taumKd[300001], taumsyn[300001];
  double mNainf[300001], hNainf[300001], mCaTinf[300001], hCaTinf[300001], mCaSinf[300001], hCaSinf[300001], mAinf[300001], hAinf[300001], mKCainf[300001], mKdinf[300001], msyninf[300001];
  double BAL_mNa[300001], BAL_hNa[300001], BAL_mCaT[300001], BAL_hCaT[300001], BAL_mCaS[300001], BAL_hCaS[300001], BAL_mA[300001], BAL_hA[300001], BAL_mKCa[300001], BAL_mKd[300001], BAL_msyn[300001], newBAL_msyn[300001];
  ...
  double amNa[300001], ahNa[300001], amCaT[300001], ahCaT[300001], amCaS[300001], ahCaS[300001], amA[300001], ahA[300001], amKCa[300001], amKd[300001], amsyn[300001];
  double bmNa[300001], bhNa[300001], bmCaT[300001], bhCaT[300001], bmCaS[300001], bhCaS[300001], bmA[300001], bhA[300001], bmKCa[300001], bmKd[300001], bmsyn[300001];
  ...
  double Vosc = 0;
  double Vzero[3]; Vzero[0]=-1; Vzero[1]=-1; Vzero[2]=-1;
  for (int ii=0;ii<300001;ii++)
    {
      V[ii] = -100+ii*0.001;
	 ...
	 Between fast and slow: 0<a<1 and b=1, between slow and ultraslow: a=0 and 0<b<1. */
      ...
      //Ca2 = 10; %1.2578 Value around spike threshold of -50mV (JG)
      ...
      Istatic[ii] = -gNa*get_power(mNainf[ii],3)*hNainf[ii]*(V[ii]-VNa) - gCaT*get_power(mCaTinf[ii],3)*hCaTinf[ii]*(V[ii]-VCa) - gCaS*get_power(mCaSinf[ii],3)*hCaSinf[ii]*(V[ii]-VCa) - gA*get_power(mAinf[ii],3)*hAinf[ii]*(V[ii]-VK) -gKCa*get_power(mKCainf[ii],4)*(V[ii]-VK) - gKd*get_power(mKdinf[ii],4)*(V[ii]-VK) - gleak*(V[ii]-Vleak) -gsyn*msyninf[ii]*(V[ii]-Vsyn) + Iapp;
      ...
      else
	{
      ...
      BAL_msyn[ii] = gsyn*(-dXinf(V[ii],40,-2)*(V[ii]-Vsyn)); //treat V1=V2 for the two neurons
      newBAL_msyn[ii] = new_gsyn*(-dXinf(V[ii],40,-2)*(V[ii]-Vsyn)); //treat V1=V2 for the two neurons
    }

  // Algorithm that computes the values of a and b
  compare_tau(amNa,bmNa,taumNa,taumNa,taumKd,tauhCaS);
  compare_tau(ahNa,bhNa,tauhNa,taumNa,taumKd,tauhCaS);
  compare_tau(amCaT,bmCaT,taumCaT,taumNa,taumKd,tauhCaS);
  compare_tau(ahCaT,bhCaT,tauhCaT,taumNa,taumKd,tauhCaS);
  compare_tau(amCaS,bmCaS,taumCaS,taumNa,taumKd,tauhCaS);
  compare_tau(ahCaS,bhCaS,tauhCaS,taumNa,taumKd,tauhCaS);
  compare_tau(amA,bmA,taumA,taumNa,taumKd,tauhCaS);
  compare_tau(ahA,bhA,tauhA,taumNa,taumKd,tauhCaS);
  compare_tau(amKd,bmKd,taumKd,taumNa,taumKd,tauhCaS);
  compare_tau(amKCa,bmKCa,taumKCa,taumNa,taumKd,tauhCaS);
  compare_tau(amsyn,bmsyn,taumsyn,taumNa,taumKd,tauhCaS);

  double BALf[300001], BALs[300001], BALu[300001];
  ...
  //Track the transcritical bifurcation.
  //Compute V_TC = V_th
  double VTC = -50; //fzero(@(V)Jx(V),-50); 
  
  //4. Compute the balance at V_SN and V_osc
  int X1 = find_coef(V,VTC);
  int X2 = find_coef(V,Vosc); 
  ...
  BAL2_mCaS = BAL_mCaS;
  BAL2_hCaS = BAL_hCaS;
  BAL2_msyn = -gsyn*dmsyn; %new synaptic conductance
  */
  ...
  A[3][0] = 1; 
  //
  A[0][1] = (bmKd[X1]-amKd[X1]) * (-4*get_power(mKdinf[X1],3)*(V[X1]-VK) * dXinf(V[X1],12.3,-11.8));
  A[1][1] = (bmKd[X2]-amKd[X2]) * (-4*get_power(mKdinf[X2],3)*(V[X1]-VK) * dXinf(V[X2],12.3,-11.8));
  A[2][1] = (1-bmKd[X1]) *  (-4*get_power(mKdinf[X1],3)*(V[X1]-VK) * dXinf(V[X1],12.3,-11.8));
  A[3][1] = -get_power(mKdinf[X1],4) * (VTC-VK);
  //
  ...
  c[0] = BALs[X1] - (bmNa[X1]-amNa[X1])*BAL_mNa[X1]  + (bhNa[X1]-ahNa[X1])*BAL_hNa[X1] + (bmCaT[X1]-amCaT[X1])*BAL_mCaT[X1] + (bhCaT[X1]-ahCaT[X1])*BAL_hCaT[X1] + (bmCaS[X1]-amCaS[X1])*BAL_mCaS[X1] + (bhCaS[X1]-ahCaS[X1])*BAL_hCaS[X1] +  (bmsyn[X1]-amsyn[X1])*newBAL_msyn[X1];
