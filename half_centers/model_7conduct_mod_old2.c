...
#define total_time 5000.0   //the time of the presynaptic input in ms
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  double Ca_inf[2]; Ca_inf[0] = 0.5; Ca_inf[1] = 0.5;
  double g[6][2];
  double gate[2][6][2];
  double tau[2][6][2];
  ...
  while (current_time < total_time)
   {       
     ...
     */
	 ...
	 Ca_inf[nn] = 0.05 - 0.94*I_Ca[nn]*10;    
     ...
     S_inf[0] = 1.0/(1.0+exp(-(V[1]+45))/2.0);
     S_inf[1] = 1.0/(1.0+exp(-(V[0]+45))/2.0);
	 ...
	 /*
	 I_ion[nn] = g_L[nn]*(V[nn]-E_L); //sum of all ionic currents
	 for (ii=0;ii<6; ii++)
	   {
	     I_ion[nn] += gnmh[ii][nn]*(V[nn]-E[ii][nn]);
	   }
	 */
