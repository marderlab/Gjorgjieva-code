...
int main(int argc, char *argv[])
{
  ...
  double Vsyn = atof(argv[5]);
  double gapp = atof(argv[6]);
  double frac_per = atof(argv[7]);
  int which_cell = atof(argv[8]);
  double phase = atof(argv[9]);
  double time_burst_start[2];
  time_burst_start[0] = atof(argv[10]);
  time_burst_start[1] = atof(argv[11]);
  double ref_period[2];
  ref_period[0] = atof(argv[12]);
  ref_period[1] = atof(argv[13]);
  ...
  char V_file [999];
  sprintf (V_file, "V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat",in1,in2,g_syn1,g_syn2,Vsyn,gapp,frac_per,phase,which_cell);
 ...
 //reversal potentials
  ...
  double V[2];
  V[0] = -50; //mV
  V[1] = -50; //start with same initial condition!
  ...
  while (current_time < total_time)
   {       
     ...
     */
	 ...
	 if (which_cell == 2) //put input into both cells
	   {
	     //printf("into both cells %d \n",nn);
	     //put in synaptic input pulse to get PRC
	     if ((current_time > time_burst_start[nn] + phase*ref_period[nn] + 5000) && (current_time < time_burst_start[nn] + phase*ref_period[nn] + frac_per*ref_period[nn] + 5000))
	       {
		 sum_g[nn] += gapp;
		 sum_Eg[nn] += gapp*Vsyn;
	       }
	   ...
	   {
	     if (nn == which_cell) //put input only into which_cell
	       {
		 //put in synaptic input pulse to get PRC
		 if ((current_time > time_burst_start[nn] + phase*ref_period[nn] + 5000) && (current_time < time_burst_start[nn] + phase*ref_period[nn] + frac_per*ref_period[nn] + 5000))
		   {
