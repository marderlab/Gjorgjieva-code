...
int main(int argc, char *argv[])
{
  ...
  double V_half =  atof(argv[5]); //half-activation, determines if synapses are graded, or spike-mediated
  double tau_syn = atof(argv[6]); //synaptic time constant
  double E_syn = atof(argv[7]); //reversal potential, determines if synapses are inhibitory or excitatory
  ...
  char stim_file [999];
  FILE *stimfile;
  sprintf (stim_file, "conductances_2_2000.dat");
  ...
  char sp_file [999];
  sprintf (sp_file, "sp_gNa_%g_%g_gCaT_%g_%g_gCaS_%g_%g_gA_%g_%g_gKCa_%g_%g_gK_%g_%g_gH_%g_%g_gL_%g_%g_gsyn_%g_%g_Vhalf_%g_tsyn_%g_Esyn_%g.dat",g_Na1,g_Na2,g_CaT1,g_CaT2,g_CaS1,g_CaS2,g_A1,g_A2,g_KCa1,g_KCa2,g_K1,g_K2,g_H1,g_H2,g_L1,g_L2,g_syn1,g_syn2,V_half,tau_syn,E_syn);
  //sprintf (sp_file, "sp_mean_%g_sig_%g_gNa_%g_%g_gCaT_%g_%g_gCaS_%g_%g_gA_%g_%g_gKCa_%g_%g_gK_%g_%g_gH_%g_%g_gL_%g_%g_gsyn_%g_%g_corr_%g_tc_%g.dat",mean,sigma,g_Na1,g_Na2,g_CaT1,g_CaT2,g_CaS1,g_CaS2,g_A1,g_A2,g_KCa1,g_KCa2,g_K1,g_K2,g_H1,g_H2,g_L1,g_L2,g_syn1,g_syn2,corr_coef,tau_c);
  FILE *spfile;
  spfile = fopen(sp_file,"w");
  
  char V_file [999];
  sprintf (V_file, "V_gNa_%g_%g_gCaT_%g_%g_gCaS_%g_%g_gA_%g_%g_gKCa_%g_%g_gK_%g_%g_gH_%g_%g_gL_%g_%g_gsyn_%g_%g_Vhalf_%g_tsyn_%g_Esyn_%g.dat",g_Na1,g_Na2,g_CaT1,g_CaT2,g_CaS1,g_CaS2,g_A1,g_A2,g_KCa1,g_KCa2,g_K1,g_K2,g_H1,g_H2,g_L1,g_L2,g_syn1,g_syn2,V_half,tau_syn,E_syn);
  //sprintf (V_file, "V_mean_%g_sig_%g_gNa_%g_%g_gCaT_%g_%g_gCaS_%g_%g_gA_%g_%g_gKCa_%g_%g_gK_%g_%g_gH_%g_%g_gL_%g_%g_gsyn_%g_%g_corr_%g_tc_%g.dat",mean,sigma,g_Na1,g_Na2,g_CaT1,g_CaT2,g_CaS1,g_CaS2,g_A1,g_A2,g_KCa1,g_KCa2,g_K1,g_K2,g_H1,g_H2,g_L1,g_L2,g_syn1,g_syn2,corr_coef,tau_c);
  FILE *Vfile;
 ...
 //reversal potentials
  ...
  /*Initial conditions*/
  double V[2];
  V[0] = -50 + gsl_rng_uniform(r); //mV
  V[1] = -50 + gsl_rng_uniform(r);
  ...
  double spike[2]; spike[0] = 0.0; spike[1] = 0.0; //to impose an absolute refractory period when reading spikes
  double rand_g1 = 0.0;
  double rand_g2 = 0.0;
  double rand_g3 = 0.0;
  ...
  while (current_time < total_time)
   {       
    ...
    for (nn=0;nn<2;nn++)
	 ...
	 if (V[nn] > V_th  & current_time>5000)
	   {
	     //if a high threshold was detected, record the spike
	     if (voltage_high[nn] == 0 && current_time-spike[nn]>5) //5 is tau_abs
	       {
		 fprintf(spfile,"%d %f \n", nn, current_time);
		 spike[nn] = current_time;
	       }
	   ...
	   {
	     if (voltage_high[nn] > 0)
	       {
		 voltage_high[nn] = 0; //reset
	       }
	   }
