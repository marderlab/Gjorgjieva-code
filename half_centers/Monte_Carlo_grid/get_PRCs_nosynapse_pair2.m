...
V_45887_0 = dlmread(sprintf('V_in1_%g_in2_%g_gsyn_0_0.dat',cell1,cell2));
V1 = V_45887_0(V_45887_0(:,1)==0,2); 
V2 = V_45887_0(V_45887_0(:,1)==1,2);
    ...
    for tt=1:length(Tdur_vec)
        ...
        for vv=1:length(Vsyn_vec)
            ...
            for frac=[0.05:0.05:0.95]                    
                ...
                x3 = intersect(x1+1,x2);
                sp2 = tot_T(x3); %get spike times
                [CV1, num_sp1, period1, burst_dur1, first_spike1, last_spike1] = get_train_properties_HCO(sp1,0,20); %13        
                [CV2, num_sp2, period2, burst_dur2, first_spike2, last_spike2] = get_train_properties_HCO(sp2,0,20); %10
...
%%
%%%% Plot PRCs for a fixed amplitude (0.05 uS and 10 uS) but vary the
%%%% duration of the pulse [2 and 5]
figure;hold on;
jj = 0;
for ii=[1:5]
    c = get_color(jj,7)
    ...
    plot(frac,PRC_cell{7,ii,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
