...
V_18745_syn = dlmread(sprintf('V_in1_312_in2_1125_gsyn_%g_%g.dat',g_syn1,g_syn2));
Vs1 = V_18745_syn(V_18745_syn(:,1)==0,2); 
Vs2 = V_18745_syn(V_18745_syn(:,1)==1,2);
...
gmax_vec = [0.01 0.05 0.2 1 10 50 100 500]; %strength of synaptic conductance
Tdur_vec = [0.05 0.1 0.2 0.3 0.4]; %duration as percent of period
Vsyn_vec = [-78 0];

%%%%%%%%%%%% INPUT INTO CELL 1 ONLY %%%%%%%%%%%%
    ...
    for tt=1:length(Tdur_vec)
        ...
        for vv=1:length(Vsyn_vec)
            ...
            for frac=[0.05:0.05:0.95]                    
                ...
                V_nosyn = dlmread(sprintf('PRC_data/V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat',cell1,cell2,g_syn1,g_syn2,Vsyn,gmax,Tdur_vec(tt),frac,0));
            ...
            PRC_cell1{gg,tt,vv} = PRC;
            PRC2_cell1{gg,tt,vv} = PRC2;
        end %end of Vsyn
    end %end of Tdur
end %end of gmax 
...
for ii=[1:8]
    ...
    %plot(frac,PRC_cell1{ii,3,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell1{ii,3,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell1{ii,3,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell1{ii,3,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
...
for ii=[1:5]
    c = get_color(jj,7)
    jj=jj+1;
    %plot(frac,PRC_cell1{5,ii,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell1{5,ii,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell1{5,ii,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell1{5,ii,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
...
for gg=1:length(gmax_vec)
    for tt=1:length(Tdur_vec)
        ...
        for vv=1:length(Vsyn_vec)
            ...
            for frac=[0.05:0.05:0.95]                    
                %sprintf('V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat',cell1,cell2,g_syn1,g_syn2,Vsyn,gmax,Tdur_vec(tt),frac,0)
                V_nosyn = dlmread(sprintf('PRC_data/V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat',cell1,cell2,g_syn1,g_syn2,Vsyn,gmax,Tdur_vec(tt),frac,1));
                V1 = V_nosyn(V_nosyn(:,1)==0,2); 
                V2 = V_nosyn(V_nosyn(:,1)==1,2);
                Vth = 15; %mV threshold for detecting spikes
                dV1 = diff(V1); 
            ...
            %namee = sprintf('PRCs_pair%g_gmax%g_Tdur%gper_Vsyn%g.mat',vec_test2(ll),gmax,Tdur_vec(tt)*100,Vsyn);
            %save(namee,'PRC','PRC2','frac');
            PRC_cell2{gg,tt,vv} = PRC;
            PRC2_cell2{gg,tt,vv} = PRC2;
        end %end of Vsyn
    end %end of Tdur
end %end of gmax 
%%
...
for ii=[1:8]
    c = get_color(jj,10); jj=jj+1;
    %plot(frac,PRC_cell2{ii,3,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell2{ii,3,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell2{ii,3,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell2{ii,3,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
...
for ii=[1:5]
    c = get_color(jj,7);
    jj=jj+1;
    %plot(frac,PRC_cell2{5,ii,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell2{5,ii,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell2{5,ii,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell2{5,ii,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
...
for gg=1:length(gmax_vec)
    for tt=1:length(Tdur_vec)
        ...
        for vv=1:length(Vsyn_vec)
            ...
            for frac=[0.05:0.05:0.95]                    
                %sprintf('V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat',cell1,cell2,g_syn1,g_syn2,Vsyn,gmax,Tdur_vec(tt),frac,0)
                V_nosyn = dlmread(sprintf('PRC_data/V_in1_%d_in2_%d_gsyn_%g_%g_Vsyn_%g_gapp_%g_fracper_%g_phase_%g_cell_%d.dat',cell1,cell2,g_syn1,g_syn2,Vsyn,gmax,Tdur_vec(tt),frac,2));
                V1 = V_nosyn(V_nosyn(:,1)==0,2); 
                V2 = V_nosyn(V_nosyn(:,1)==1,2);
                Vth = 15; %mV threshold for detecting spikes
                dV1 = diff(V1); 
            ...
            %namee = sprintf('PRCs_pair%g_gmax%g_Tdur%gper_Vsyn%g.mat',vec_test2(ll),gmax,Tdur_vec(tt)*100,Vsyn);
            %save(namee,'PRC','PRC2','frac');
            PRC_cell12{gg,tt,vv} = PRC;
            PRC2_cell12{gg,tt,vv} = PRC2;
        end %end of Vsyn
    end %end of Tdur
end %end of gmax 
...
for ii=[1:8]
    c = get_color(jj,10); jj=jj+1;
    %plot(frac,PRC_cell12{ii,3,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell12{ii,3,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell12{ii,3,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell12{ii,3,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
...
for ii=[1:5]
    c = get_color(jj,7);
    jj=jj+1;
    %plot(frac,PRC_cell12{5,ii,1}(:,1),'o-','linewidth',2,'color',c); %cell 1, inh
    %plot(frac,PRC_cell12{5,ii,1}(:,2),'o-','linewidth',2,'color',c); %cell 2, inh
    %plot(frac,PRC_cell12{5,ii,2}(:,1),'o-','linewidth',2,'color',c); %cell 1, exc
    plot(frac,PRC_cell12{5,ii,2}(:,2),'o-','linewidth',2,'color',c); %cell 2, exc
end
