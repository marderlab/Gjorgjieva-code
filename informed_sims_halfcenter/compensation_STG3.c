...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  c[0] = BALs[X1] - ((bmNa[X1]-amNa[X1])*BAL_mNa[X1]  + (bhNa[X1]-ahNa[X1])*BAL_hNa[X1] + (bmCaT[X1]-amCaT[X1])*BAL_mCaT[X1] + (bhCaT[X1]-ahCaT[X1])*BAL_hCaT[X1] + (bmCaS[X1]-amCaS[X1])*BAL_mCaS[X1] + (bhCaS[X1]-ahCaS[X1])*BAL_hCaS[X1] + (bmH[X1]-amH[X1])*BAL_mH[X1] +  (bmsyn[X1]-amsyn[X1])*newBAL_msyn[X1]);
  
  c[1] = BALs[X2] - ((bmNa[X2]-amNa[X2])*BAL_mNa[X2]  + (bhNa[X2]-ahNa[X2])*BAL_hNa[X2]  +  (bmCaT[X2]-amCaT[X2])*BAL_mCaT[X2] + (bhCaT[X2]-ahCaT[X2])*BAL_hCaT[X2] + (bmCaS[X2]-amCaS[X2])*BAL_mCaS[X2] + (bhCaS[X2]-ahCaS[X2])*BAL_hCaS[X2] + (bmH[X2]-amH[X2])*BAL_mH[X2] + (bmsyn[X2]-amsyn[X2])*newBAL_msyn[X2]);
  
  c[2] = BALu[X1]  - ((1-bmNa[X1])*BAL_mNa[X1]  + (1-bhNa[X1])*BAL_hNa[X1] + (1-bmCaT[X1])*BAL_mCaT[X1] + (1-bhCaT[X1])*BAL_hCaT[X1] + (1-bmCaS[X1])*BAL_mCaS[X1] + (1-bhCaS[X1])*BAL_hCaS[X1] + (1-bmH[X1])*BAL_mH[X1] + (1-bmsyn[X1])*newBAL_msyn[X1]);
  
  c[3] = Istatic[X1] - (-gNa*get_power(mNainf[X1],3)*hNainf[X1]*(V[X1]-VNa) -gCaT*get_power(mCaTinf[X1],3)*hCaTinf[X1]*(V[X1]-VCa) -gCaS*get_power(mCaSinf[X1],3)*hCaSinf[X1]*(V[X1]-VCa) -gH*mHinf[X1]*(V[X1]-VH) -new_gsyn*msyninf[X1]*(V[X1]-Vsyn) -gleak*(V[X1]-Vleak));
