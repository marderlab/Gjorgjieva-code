...
#define total_time 20000.0   //the time of the presynaptic input in ms
...
int main(int argc, char *argv[])
{
  ...
  //for cell 2
  double g_Na2 = g_Na1;//atof(argv[11]);
  double g_CaT2 = g_CaT1;//atof(argv[12]);
  double g_CaS2 = g_CaS1;//atof(argv[13]); 
  double g_A2 = g_A1;//atof(argv[14]);
  double g_KCa2 = g_KCa1;//atof(argv[15]);
  double g_K2 = g_K1;//atof(argv[16]);
  double g_H2 = g_H1;//atof(argv[17]);
 ...
 //reversal potentials
  ...
  double V_half = 40.0;   //-45; //mV
  double V_slope = -2.0;  //5; //mV
