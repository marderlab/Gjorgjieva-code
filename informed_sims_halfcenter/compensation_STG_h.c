...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  //
  A[0][2] = (bmH[X1]-amH[X1]) * (-dXinf(V[X1],70.0,6.0) * (V[X1]-VH));
  A[1][2] = (bmH[X2]-amH[X2]) * (-dXinf(V[X2],70.0,6.0) * (V[X2]-VH));
  A[2][2] = (1-bmH[X1]) * (-dXinf(V[X1],70.0,6.0) * (V[X1]-VH));
  A[3][2] = -mHinf[X1]*(VTC-VH);
  //
  ...
  */
  ...
  c[0] = BALs[X1] - ((bmNa[X1]-amNa[X1])*BAL_mNa[X1]  + (bhNa[X1]-ahNa[X1])*BAL_hNa[X1] + (bmCaT[X1]-amCaT[X1])*BAL_mCaT[X1] + (bhCaT[X1]-ahCaT[X1])*BAL_hCaT[X1] + (bmCaS[X1]-amCaS[X1])*BAL_mCaS[X1] + (bhCaS[X1]-ahCaS[X1])*BAL_hCaS[X1] + (bmA[X1]-amA[X1])*BAL_mA[X1]  + (bhA[X1]-ahA[X1])*BAL_hA[X1] +  (bmsyn[X1]-amsyn[X1])*newBAL_msyn[X1]);
  
  c[1] = BALs[X2] - ((bmNa[X2]-amNa[X2])*BAL_mNa[X2]  + (bhNa[X2]-ahNa[X2])*BAL_hNa[X2]  +  (bmCaT[X2]-amCaT[X2])*BAL_mCaT[X2] + (bhCaT[X2]-ahCaT[X2])*BAL_hCaT[X2] + (bmCaS[X2]-amCaS[X2])*BAL_mCaS[X2] + (bhCaS[X2]-ahCaS[X2])*BAL_hCaS[X2] + (bmA[X2]-amA[X2])*BAL_mA[X2]  + (bhA[X2]-ahA[X2])*BAL_hA[X2] + (bmsyn[X2]-amsyn[X2])*newBAL_msyn[X2]);
  
  c[2] = BALu[X1]  - ((1-bmNa[X1])*BAL_mNa[X1]  + (1-bhNa[X1])*BAL_hNa[X1] + (1-bmCaT[X1])*BAL_mCaT[X1] + (1-bhCaT[X1])*BAL_hCaT[X1] + (1-bmCaS[X1])*BAL_mCaS[X1] + (1-bhCaS[X1])*BAL_hCaS[X1] + (1-bmA[X1])*BAL_mA[X1]  + (1-bhA[X1])*BAL_hA[X1] + (1-bmsyn[X1])*newBAL_msyn[X1]);
  
  c[3] = Istatic[X1] - (-gNa*get_power(mNainf[X1],3)*hNainf[X1]*(V[X1]-VNa) -gCaT*get_power(mCaTinf[X1],3)*hCaTinf[X1]*(V[X1]-VCa) -gCaS*get_power(mCaSinf[X1],3)*hCaSinf[X1]*(V[X1]-VCa) -gA*get_power(mAinf[X1],3)*hAinf[X1]*(V[X1]-VK) -new_gsyn*msyninf[X1]*(V[X1]-Vsyn) -gleak*(V[X1]-Vleak));
