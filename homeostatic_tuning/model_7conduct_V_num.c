...
int main(int argc, char *argv[])
{
  ...
  double corr_coef = atof(argv[3]); //correlation coefficient of the input
  double tau_c = atof(argv[4]); //correlation time const
  double g1_vec[9];
  double g2_vec[9];
  ...
  while (num_run<51)
    {
      sprintf (stim_file, "g1_target_30_taug_100_taum_1000_num_%d.dat", num_run);
      //sprintf (stim_file, "g1_target_10_taug_100_taum_1000_num_%d.dat", num_run);
      stimfile = fopen(stim_file,"r");
      read_stimulus(stimfile,g1_vec);
      fclose(stimfile);

      sprintf (stim_file, "g2_target_30_taug_100_taum_1000_num_%d.dat", num_run);
      //sprintf (stim_file, "g2_target_10_taug_100_taum_1000_num_%d.dat", num_run);
      stimfile = fopen(stim_file,"r");
	...
	spfile2 = fopen(sp_file2,"w");
      */
      ...
      char V_file1 [999];
      sprintf (V_file1, "V1_mean_%g_sig_%g_num_%d.dat", mean, sigma, num_run);
      FILE *Vfile1;
      Vfile1 = fopen(V_file1,"w");
      //
      char V_file2 [999];
      sprintf (V_file2, "V2_mean_%g_sig_%g_num_%d.dat", mean, sigma, num_run);
      FILE *Vfile2;
