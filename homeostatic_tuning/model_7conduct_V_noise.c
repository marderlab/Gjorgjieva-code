...
int main(int argc, char *argv[])
{
  ...
  while (num_run<Ntot+0.01)
    {
      sprintf (stim_file, "g1_mean_%g_sig_%g_tau_%g_coef_%g_target_%g_taug_%g_taum_%g_num_%d.dat",mean,sigma,tau_c,corr_coef,target,taug,taum,num_run);
      stimfile = fopen(stim_file,"r");
      read_stimulus(stimfile,g1_vec);
      fclose(stimfile);

      sprintf (stim_file, "g2_mean_%g_sig_%g_tau_%g_coef_%g_target_%g_taug_%g_taum_%g_num_%d.dat",mean,sigma,tau_c,corr_coef,target,taug,taum,num_run);
      stimfile = fopen(stim_file,"r");
	...
	spfile2 = fopen(sp_file2,"w");
      */
      ...
      char V_file1 [999];
      sprintf (V_file1, "V1_mean_%g_sig_%g_tau_%g_coef_%g_num_%d.dat", mean, sigma, tau_c, corr_coef, num_run);
      FILE *Vfile1;
      Vfile1 = fopen(V_file1,"w");
      //
      char V_file2 [999];
      sprintf (V_file2, "V2_mean_%g_sig_%g_tau_%g_coef_%g_num_%d.dat", mean, sigma, tau_c, corr_coef, num_run);
      FILE *Vfile2;
