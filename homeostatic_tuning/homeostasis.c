...
int main(int argc, char *argv[])
{
  ...
  double taug = 100;
  double taum = 10*taug;
  double tstop = 600*taug;
  double ms_per_samp = tstop/1000.0;
  ...
  */
  ...
  char sp_file1 [999];
  sprintf (sp_file1, "g1_target_%g_taug_%g_taum_%g.dat",target,taug,taum);
  FILE *spfile1;
  spfile1 = fopen(sp_file1,"w");
  //
  char sp_file2 [999];
  sprintf (sp_file2, "g2_target_%g_taug_%g_taum_%g.dat",target,taug,taum);
  FILE *spfile2;
 ...
 //reversal potentials
  double E_Na = 50; //mV or 30 - to reproduce some of Tim's figures, need to make this 30 mV
  double E_A = -80; //mV
  double E_K = -80; //mV
  double E_H = -20; //mV
  ...
  S_inf[0] = Svar[0]; 
  S_inf[1] = Svar[1];
  double g_syn[2]; g_syn[0] = gsyn; g_syn[1] = gsyn;
